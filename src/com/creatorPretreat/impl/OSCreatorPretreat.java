...
public class OSCreatorPretreat implements ICreatorPretreat {
	...
	public void insertRecord(JSONObject liveJSONObject) {
		JSONObject jsonObject = liveJSONObject.getJSONObject("os_version");
		for(Object keyObj: jsonObject.keySet()){
			String key = keyObj.toString();
			if(osVersionMap.containsKey(key)){
				osVersionMap.put(key, osVersionMap.get(key) + jsonObject.getInt(key));
			}else{
				m_tab.put(key, new BaseCols());
				osVersionMap.put(key, jsonObject.getInt(key));
			}
