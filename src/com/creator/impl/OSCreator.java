...
public class OSCreator implements ICreator {

	Map<String, BaseCols> m_tab = new TreeMap<String, BaseCols>();
	Map<String, Integer> osVersionMap = new TreeMap<String, Integer>();

	@Override
	public void insertRecord(PlayDataEntity liveinfo) {
		String osVersion = liveinfo.getOs_version() + "";
		if (null != m_tab.get(osVersion)) {
			m_tab.get(osVersion).add(liveinfo);
			osVersionMap.put(osVersion, osVersionMap.get(osVersion) + 1);
		} else {
			m_tab.put(osVersion, new BaseCols());
			osVersionMap.put(osVersion, 1);
			m_tab.get(osVersion).add(liveinfo);
		}
	}
	...
	public Object getRowList() {
		...
		for(String key : osVersionMap.keySet()){
			if(key.equals(notc)){
				notComplete = osVersionMap.get(notc);
				m_tab.remove(key);
				continue;
			}
			total += osVersionMap.get(key);
		}
		for(String key : osVersionMap.keySet()){
			if(notComplete != 0){
				if(!key.equals(notc)){
					BigDecimal btemp = new BigDecimal(osVersionMap.get(key) + notComplete * (osVersionMap.get(key) / (double)total)).setScale(0, BigDecimal.ROUND_HALF_UP);
					int itemp = Integer.parseInt(btemp.toString());;
					osVersionMap.put(key, itemp);
				}
			}
		}
		...
		for (String key : m_tab.keySet()) {
			...
			entity_count.setName("osVersion_NUM");
			entity_count.setValue(osVersionMap.get(key));
			valuesCount.put("sample_number", entity_count);

			countCol.setValues(valuesCount);
			countCol.setName("次数");
			countCol.setDescription("出现该操作系统的播放次数");
