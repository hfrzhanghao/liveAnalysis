...
public class DeviceCreator implements ICreator {
	...
	public void insertRecord(PlayDataEntity liveinfo) {
		String os_type = liveinfo.getOs_type() + "";
		if (null != m_tab.get(os_type)) {
			m_tab.get(os_type).add(liveinfo);
			osTypeMap.put(os_type, osTypeMap.get(os_type) + 1);
		} else {
			m_tab.put(os_type, new BaseCols());
			osTypeMap.put(os_type, 1);
			m_tab.get(os_type).add(liveinfo);
		}
	}
